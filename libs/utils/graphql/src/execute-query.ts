...
export type ExecuteQueryResult<QueryResult> = Future<Result<QueryResult, ExecuteQueryError>>
...
export function executeQuery<
  ...
  return Future.make((resolve) => {
    ...
    client.query<QueryResult, Variables>({
      query: replacedQuery,
      variables,
      context: {
        fetchOptions: {
      ...
      .then(result => {
        if (result.errors?.length) {
          resolve(Result.Error<QueryResult, ExecuteQueryError>(new ApolloError({ graphQLErrors: result.errors })));
        }
      ...
      .catch(error => {
        resolve(Result.Error<QueryResult, ExecuteQueryError>(new UnknownError(error)));
      });
