...
import { NcImage, ButtonClose, NextPrev, createGlide } from '../../atoms'
import { useSafeDocument } from '../../hooks/useSafeDocument'
...
export const ModalPhotos: FC<ModalPhotosProps> = ({
  imgs,
  isOpen,
  onClose,
  initFocus = 0,
  ...
  const safeDocument = useSafeDocument()
  const id = useId()
  ...
  const MY_GLIDEJS = useMemo(() => {
    return createGlide(`.${UNIQUE_CLASS}`, {
      direction:
        safeDocument.querySelector('html')?.getAttribute('dir') === 'rtl'
          ? 'rtl'
          : 'ltr',
      gap: 10,
      perView: 1,
