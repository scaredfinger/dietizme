...
function SignIn() {
  ...
  const { currentUser, isAuthenticated, signInError, isSigningIn, login, isLoading } = useAuth()
  ...
  useEffect(() => {
    console.log('FLOW', 'SignIn', { isAuthenticated, push })
    if (isAuthenticated) {
      console.log('FLOW', 'SignIn', 'isAuthenticated')
      dispatch(logInAction(() => push('/admin')))
    }
            ...
            >
              ...
              {signInError && (
                <p className="text-danger mb-10 text-center text-base">
                  {signInError.message} - { signInError.error }
                </p>
              )}
