          ...
          <div className="overflow-x-auto shadow-md rounded-lg">
            <table className="min-w-full bg-white">
              <thead className="bg-gray-100">
                <tr>
                  <th className="py-3 px-4 text-left font-semibold text-gray-700">{t('ID')}</th>
                  <th className="py-3 px-4 text-left font-semibold text-gray-700">{t('Name')}</th>
                ...
                {data.product_group.map((group) => (
                  <tr key={group.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <button 
                        onClick={() => push(`/admin/product-groups/${group.id}`)}
                        className="text-blue-600 hover:text-blue-800 mr-2"
                      >
                        {t('View')}
                      </button>
                      <button 
                        onClick={() => push(`/admin/product-groups/${group.id}/edit`)}
